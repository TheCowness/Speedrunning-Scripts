...
//execute("Select count(*) FROM dragonwarrioriv_characterstatdata");
//var_dump(get());
...
if(
	...
	if($HP !== ''){
		$query .= ",(SELECT count(*) FROM dragonwarrioriv_characterstatdata".$where_clause." AND HP >= ".$HP.") as hp_percentile";
	}else{
		$query .= ",-1 as hp_percentile";
	}
	if($MP !== ''){
		$query .= ",(SELECT count(*) FROM dragonwarrioriv_characterstatdata".$where_clause." AND MP >= ".$MP.") as mp_percentile";
	}else{
		$query .= ",-1 as mp_percentile";
	}
	if($Str !== ''){
		$query .= ",(SELECT count(*) FROM dragonwarrioriv_characterstatdata".$where_clause." AND Str >= ".$Str.") as str_percentile";
	}else{
		$query .= ",-1 as str_percentile";
	}
	if($Agi !== ''){
		$query .= ",(SELECT count(*) FROM dragonwarrioriv_characterstatdata".$where_clause." AND Agi >= ".$Agi.") as agi_percentile";
	}else{
		$query .= ",-1 as agi_percentile";
	}
	if($Vit !== ''){
		$query .= ",(SELECT count(*) FROM dragonwarrioriv_characterstatdata".$where_clause." AND Vit >= ".$Vit.") as vit_percentile";
	}else{
		$query .= ",-1 as vit_percentile";
	}
	if($Int !== ''){
		$query .= ",(SELECT count(*) FROM dragonwarrioriv_characterstatdata".$where_clause." AND `Int` >= ".$Int.") as int_percentile";
	}else{
		$query .= ",-1 as int_percentile";
	}
	if($Luck !== ''){
		$query .= ",(SELECT count(*) FROM dragonwarrioriv_characterstatdata".$where_clause." AND Luck >= ".$Luck.") as luck_percentile";
	}else{
		$query .= ",-1 as luck_percentile";
	}
	
	$query .= " FROM dragonwarrioriv_characterstatdata";
...
This script compares against 1000 trials of leveling up each character from level 1 to level 98 (not 99; deal with it).  Submit the form below to get the possible ranges of each character stat for the selected character and experience level.<br>
<br>
...
<br>
I have another page set up <a href="DW4_Stat_Deviations.php">here</a> that compares against 10,000 trials, but it loads a lot more slowly and isn't really more accurate.  I recommend using this page instead.<br>
<br>
